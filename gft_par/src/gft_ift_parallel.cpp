	...
	namespace ift
	{
		...
		*/
		...
		void OIFT(sScene32 *W,
				  ...
				  sScene32 *label)
		{
			gft::DebugTiming::start("OIFT-initialization");
			sPQueue32 *Q = NULL;
			int i, j, p, q, n;
			int w, Wmax;
			sScene32 *value;
			...
			Scene32::Destroy(&value);
			PQueue32::Destroy(&Q);
			gft::DebugTiming::end("OIFT-initialization");
					  ...
					  sScene32 *label)
			{
				...
				Wmax = MAX(gft::Scene32::GetMaximumValue(Wx),
						   MAX(gft::Scene32::GetMaximumValue(Wy),
							   gft::Scene32::GetMaximumValue(Wz))) *
					   2;
				Wmax *= (1.0 + fabsf(per) / 100.0);
				Q = PQueue32::Create(Wmax + 2, n, value->data);
							 ...
							 int niter)
			{
				...
				{
					if (label->data[p] == NIL)
						value->data[p] = sg->Wmax * 2 + 2; // sg->Wmax+1; //INT_MAX;
					else
						value->data[p] = 0;
				}
							 ...
							 int niter)
			{
				...
				for (p = 0; p < n; p++)
				{
					if (label->data[p] == NIL)
						...
						value->data[p] = sg->Wmax + 1; // INT_MAX;
				}
								  ...
								  sImage32 *label_oift)
			{
				...
				for (i = 1; i <= S[0]; i++)
				{
					if (label->data[S[i]] == 1)
					{
						p = S[i];
						q = pred->data[p];
						while (q != NIL && (label->data[q] != 1))
						{ // enquanto o predecessor nao for raiz(i.e.,Si1) e seu rótulo nao for marcado antes(i.e., NIL)
							Set::Insert(&newSi, q);
							// printf("newSi->elem: %i\n",newSi->elem);
							// printf("p: %i\n",q);
							label->data[q] = 1;
			...
			*/
									   ...
									   int *i_inv)
			{
				...
				for (i = 1; i < A->n; i++)
				{
						...
						v.y >= 0 && v.y < L->nrows)
					{
						...
						{
							if (Q->L.elem[t].color == GRAY)
							{ // mais um daqueles erros que não acontece nunca...
								L->data[t] = 1;
								PQueue32::FastRemoveElem(Q, t);
							...
							else
							{
								...
								if (Q->L.elem[t].color == BLACK)
									black++;
								else if (Q->L.elem[t].color == WHITE)
									white++;
								else
									printf("Error!\n");
							}
						...
						while (change_Qt == true)
						{
									...
									delta))
							{
								...
								if (Q->L.elem[t].color != BLACK)
								{
									...
									if (Q->L.elem[t].color == GRAY)
										PQueue32::FastRemoveElem(Q, t);
									if (Qe->L.elem[t].color == GRAY)
										PQueue32::FastRemoveElem(Qe, t);
									BB_OIFT_Propagate_bkg(t, G, Q, Qe, V, L, i_inv);
								}
								else if (L->data[t] == 1)
									printf("Erro 3:\n");
						 ...
						 sImage32 *label)
			{
			  ...
			  or 2|0|1 = exclusion between layer 0 and layer 1*/
				...
				for (i = 1; i <= S[0]; i++)
				{
					...
					{ // it is background seed, then transfer for all layers
						for (j = 0; j < nlayers; j++)
						{
							p = x + y * ncols + nrows * ncols * (j);
							lg_label[p] = 0;
				...
				for (i = 0; i < nlayers - 1; i++)
				{
					...
					{
						if (hr[i] == hr[j])
						{ // same father -> brothers -> exclusion
						 ...
						 int Wmax, int *S, int *L, int *hr)
			{
				...
				{
					if (L[p] == NIL)
						value[p] = INT_MAX; //(int)floor(FLT_MAX+0.5);
					else
						value[p] = 0;
				}
			...
			void ORelax_1(sAdjRel3 * A,
						  ...
						  int ntimes)
			{
				gft::DebugTiming::start("ORelax_1-preparation");
				sScene32 *mask;
				float *flabel_1, *flabel_2, *tmp;
				int *mask_nodes;
				...
				gft::DebugTiming::end("ORelax_1-preparation");

				while (ntimes > 0)
				{
					gft::DebugTiming::start("ORelax_1-iteration");
					// Update flabel:
					memcpy(flabel_2, flabel_1, n * sizeof(float));
// OpenMP parallel optimization: each thread processes independent mask nodes
#pragma omp parallel for private(p, u, v, q, i, sw, w, per_pq) schedule(dynamic)
					for (j = 1; j <= mask_nodes[0]; j++)
					{
						p = mask_nodes[j];
						u.c.x = gft::Scene32::GetAddressX(label, p);
						...
						for (i = 1; i < A->n; i++)
						{
							...
							if (gft::Scene32::IsValidVoxel(label, v))
							{
								...
								w = Wmax - w;
								// Optimized: single pow() instead of 3 multiplications
								float w2 = w * w; // w^2
								w = w2 * w2 * w2; // w^8 = (w^2)^3
								w /= Dpq[i];
				...
				else
				{
					...
					if (shapepriors != 0)
					{
						...
						else
						{
							...
							{
							case 1: // Circle
								raw_map = Image32::Read("./templates/circle.pgm");
								break;
							case 2: // Square
								raw_map = Image32::Read("./templates/square.pgm");
								break;
							case 3:
								raw_map = Image32::Read("./templates/ellipse2:1.pgm");
								break;
							case 4:
								raw_map = Image32::Read("./templates/ellipse3:1.pgm");
								break;
							case 5:
								raw_map = Image32::Read("./templates/rectangle2:1.pgm");
								break;
							case 6:
								raw_map = Image32::Read("./templates/rectangle3:1.pgm");
								break;
							case 7:
					...
					if (shapepriors == 1 && costtemplate == 0)
					{ // Star Convexity
						ImageGraph::Orient2DigraphOuter(sg, P_sum);
					}
					else if (shapepriors == 2)
					{ // Hedgehog
						if (R <= 1.6 && sg->A->n == 9)
					...
					else if (shapepriors < 4 && g == NULL)
					{
						...
						case 0:
							...
							break;
						case 1:
							EOIFT(sg, NULL, label, 0);
							break;
						case 2:
							OIFT_Heap(sg, NULL, label);
							break;
						case 3:
							EOIFT_Heap(sg, NULL, label, 0.0);
							break;
						case 4:
							OIFT_TZ2Bkg(sg, NULL, label);
							break;
						case 5:
							OIFT_TZ2Obj(sg, NULL, label);
							break;
						case 6:
							EOIFT_Heap_2(sg, NULL, label, 0.0);
							break;
						}
						gettimeofday(&toc, NULL);
						totaltime = (toc.tv_sec - tic.tv_sec) * 1000.0 + (toc.tv_usec - tic.tv_usec) * 0.001;
					...
					else if (shapepriors < 4 && g != NULL)
					{
						...
						{
						case 0:
							OIFT(g, transpose, NULL, label->data);
							break;
						case 1:
							EOIFT(g, transpose, NULL, label->data, 0);
							break;
						case 2:
							OIFT_Heap(g, transpose, NULL, label->data);
							break;
						case 3:
							EOIFT_Heap(g, transpose, NULL, label->data, 0.0);
							break;
						case 6:
							EOIFT_Heap_2(g, transpose, NULL, label->data, 0.0);
							break;
						}
						gettimeofday(&toc, NULL);
						totaltime = (toc.tv_sec - tic.tv_sec) * 1000.0 + (toc.tv_usec - tic.tv_usec) * 0.001;
			  ...
			  if( isOIFT(sg, S, Slabel, label) )
				...
				*/
				...
				if (g != NULL)
					Graph::Destroy(&g);
				if (transpose != NULL)
					Graph::Destroy(&transpose);
				if (C_sum != NULL)
					Image32::Destroy(&C_sum);
				if (P_sum != NULL)
					Image32::Destroy(&P_sum);
				if (pred != NULL)
					Image32::Destroy(&pred);
				if (cost != NULL)
					Image32::Destroy(&cost);
				free(S1);
			}
			  ...
			  temp->data[p] = energy[sp];
				...
				*/
				...
				if (S != NULL)
					gft::FreeIntArray(&S);
				if (energy != NULL)
					gft::FreeIntArray(&energy);
				if (label != NULL)
					gft::FreeIntArray(&label);
				if (pred != NULL)
					gft::FreeIntArray(&pred);
				if (maxorder != NULL)
					gft::FreeIntArray(&maxorder);
				if (cost1 != NULL)
					gft::FreeFloatArray(&cost1);
				//--------------------------------------------
