/**
 * @file oiftrelax_fmax_gpu.cpp
 * @brief CLI for GPU fmax-OIFT segmentation.
 *
 * Same interface as oiftrelax but uses GPU with bottleneck path cost.
 * Usage: oiftrelax_fmax_gpu <volume> <seeds> <pol> <niter> <percentile> <output> [boundary_stride] [pol_file]
 */
...
{
    if (argc < 7) {
        fprintf(stderr, "usage: oiftrelax_fmax_gpu <volume> <seeds> <pol> <niter> <percentile> <output> [boundary_stride] [pol_file]\n");
        return 1;
    }
    ...
    // percentile = argv[5] (unused for multi-label)
    ...
    if (!use_per_class) {
        // Global polarity: BG gets -pol, FG gets +pol (same as OIFT_Multi)
    ...
    // Gaussian blur 2× (CPU — same as oiftrelax)
    ...
    // Build seed index array (just the voxel indices, no count prefix)
    ...
    printf("fmax-OIFT (GPU): %.1f ms, %d iterations\n", oift_ms, iters);
