// exp_26connect.cpp
// Experiment 4: 26-connectivity adjacency
// Uses AdjRel3::Spheric(1.75) instead of Spheric(1.0) to get 26-connected neighborhood.
// Everything else is identical to the baseline oiftrelax.cpp.
...
{
    std::cout << "=== EXP 4: 26-connectivity adjacency ===" << std::endl;
    std::cout << "Using AdjRel3::Spheric(1.75) for 26-connected neighborhood" << std::endl;
    ...
    if (argc < 7)
    {
        ...
        fprintf(stdout, "exp_26connect <volume> <file_seeds> <pol> <niter> <percentile> <output_file> [boundary_stride] [pol_file]\n");
        exit(0);
    }

    // 26-connected: radius 1.75 includes all 26 face/edge/corner neighbors
    A = gft::AdjRel3::Spheric(1.75f);
    std::cout << "Adjacency size: " << A->n << " neighbors" << std::endl;
    ...
    DebugTimer::getInstance().startEvent("OIFT_Multi (26-connected)");
    if (use_per_class)
    {
    ...
    DebugTimer::getInstance().endEvent("OIFT_Multi (26-connected)");

    DebugTimer::getInstance().startEvent("ORelax_1_Multi (26-connected)");
    if (use_per_class)
    {
        int ml = (int)per_class_vec.size() - 1;
    ...
    DebugTimer::getInstance().endEvent("ORelax_1_Multi (26-connected)");
